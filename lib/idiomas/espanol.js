...
const smsRowner = () => { return `\`\`\`⛃➭⛔ ∫ Este comando está prohibido para miembros comunes.\`\`\`` }//NUMERO DE BOT
const smsOwner = () => { return `\`\`\`⛃➭⛔ ∫ Este comando sólo mi creadeor lo puede usar.\`\`\`` }//OWNER
const smsMods = () => { return `\`\`\`⛃➭⛔ ∫ Este comando sólo mods y admins del bot lo pueden usar.\`\`\`\`` }//MODERADORES
const smsPremium = () => { return `\`\`\`⛃➭⛔ ∫ Este comando sólo está disponible para usuarios premium y mi creador.\`\`\`` }//USUARIOS PREMIUM
const smsGroup = () => { return `\`\`\`⛃➭⛔ ∫ Este comando sólo se puede utilizar en grupos.\`\`\`` }//PARA GRUPOS
const smsPrivate = () => { return `\`\`\`⛃➭⛔ ∫ Este comando sólo se puede usar en chats privados.\`\`\`` }//AL PRIVADO
const smsAdmin = () => { return `\`\`\`⛃➭⛔ ∫ Este comando es solo para administradores.\`\`\`` }//ADMINS
const smsBotAdmin = () => { return `\`\`\`🪀➭⛔ ∫ Necesito ser administrador para ejecutar este comando.\`\`\`` }//BOT CON ADMIN
const smsUnreg = () => { return `⛃➭👤×⛔ ∫ Necesitas estar registrado para usar este comando. Envía "#registrar".` }//VERIFICAR
...
const smsLista1 = () => { return `🌟 INFORMACIÓN DE SodicaBot 🌟`}
...
const smsLista7 = () => { return `🐈 INSTALAR SodicaBot 🐈`}
...
//_allantilink.js
const smsTextoYT = () => { return 'SodicaBot - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 '} 
...
const smsCont7 = () => { return `${global.lenguajeGB['smsAvisoAG']()}*NO TIENE DIAMANTES 💎*`}
...
const eGataTickers = () => { return '🎫 Tickets' } 
...
const smsModP2 = () => { return `Habilitar función para que todos puedan usar SodicaBot`}
const smsModAd1 = () => { return `🛂 MODO ADMIN`}
const smsModAd2 = () => { return `Solo los Admins podrán usar SodicaBot en Grupos`}
...
//propietario(a.js
const smsPropban1 = (usedPrefix, command, bot) => { return `*ETIQUETE A ALGUIEN O RESPONDA AL MENSAJE DEL USUARIO O ESCRIBA EL NÚMERO QUE QUIERE BANEAR DE LOS COMANDOS*`}
...
const smsPropban6 = (number, usr) => { return `*@${number} ESTAS BANEADO/A POR @${usr} NO PUEDES USAR LOS COMANDOS HASTA QUE ALGUIEN REVIERTA EL BANEO* 😿`}
...
const smsJBInfo2 = () => { return `💖 *La función es estable, sí presenta algún inconveniente Comuníquese al correo: centerSodicaBot@gmail.com*\n💝 *Puede hacer una Donación voluntaria por PayPal: ${global.paypal}*\n\n*Muchas Gracias por el apoyo a ${global.packname}*`}
...
const smsContacto4 = () => { return 'Contacto Oficial de SodicaBot 🐈'}
