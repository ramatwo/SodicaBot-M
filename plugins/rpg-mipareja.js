let handler = async (m, { conn, usedPrefix }) => { // @${global.db.data.users[m.sender].pasangan.split('@')[0]}
    ...
    if (global.db.data.users[m.sender].pasangan == "") return await conn.reply(m.chat, `*${usuario}* no tenés pareja.`, fkontak,  m)
    ...
    if (global.db.data.users[global.db.data.users[m.sender].pasangan].pasangan == m.sender) return await conn.reply(m.chat, `*${usuario}* 💗 estás en una relación con *${await conn.getName(persona)}* 💖`, fkontak, m, {contextInfo: { mentionedJid: [ m.sender, who ] }})
