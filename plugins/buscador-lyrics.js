...
const handler = async (m, { conn, text, usedPrefix, command }) => {
    const teks = text ? text.trim() : (m.quoted && m.quoted.text ? m.quoted.text.trim() : '');
    ...
    if (!teks) {
        throw `${lenguajeGB['smsAvisoMG']()}${mid.smsMalused3}\n*${usedPrefix + command} Billie Eilish bored*`;
    }

    try {
        const result = await getTracks(teks);
        const lyrics = await find_lyrics(`${result[0].artist} ${result[0].title}`);
        const res = await fetch(`https://some-random-api.com/lyrics?title=${encodeURIComponent(result[0].artist + result[0].title)}`);
