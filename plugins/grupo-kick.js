let handler = async (m, { conn, participants, command, usedPrefix }) => { 
    try{
    let texto = `${lenguajeGB['smskick1']()}${usedPrefix + command} @usuario*`
    ...
    if(conn.user.jid.includes(mentioned)) return conn.reply(m.chat, `${lenguajeGB['smskick1']()}${usedPrefix + command} @usuario*`, fkontak, m)
    ...
    handler.help = ['kick']
    handler.tags = ['group']
    handler.command = /^(kick|cumear|hechar|sacar|ban)$/i
