let handler = async (m, { conn, text, usedPrefix, command}) => {
    ...
    if (users[user].afkReason === true) conn.reply(m.chat, lenguajeGB.smsPropban4(number), null, { mentions: [user] }) 
    users[user].afkReason = "soy gay y me la como doblada"
    //users[user].afk = -1
    usr = m.sender.split('@')[0]     
    await conn.reply(m.chat, "afk modificado 😛😛", null, { mentions: [user] }) 
    ...
    handler.command = /^modificarafk|cambiarafk$/i
    handler.rowner = true
