...
global.prefix = new RegExp('^[' + (opts['prefix'] || '*/!#:;,.\\-.@').replace(/[|\\{}()[\]^$+*?.\-\^]/g, '\\$&') + ']') //PREIFJO DEL BOT
...
global.authFile = `SodicaBotSession`
...
return { conversation: "hello, i'm SodicaBot-M" }},   
msgRetryCounterMap,
logger: pino({ level: 'silent' }),
auth: state,
browser: ['SodicaBot-M','Edge','1.0.0'],
version   
}       
...
logger: pino({ level: 'silent' }),
auth: state,
browser: ['SodicaBot-M','Edge','107.0.1418.26'],
version   
}*/
...
let prekey = []
let directorio = readdirSync("./SodicaBotSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') || file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-')
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./SodicaBotSession/${files}`)
})
} 
...
const directories = ['./SodicaBotSession/', './GataJadiBot/']
