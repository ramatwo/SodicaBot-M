...
text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Bienvenido, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'SodicaBot+') :
(chat.sBye || this.bye || conn.bye || 'Nos vimos @user!')).replace('@user', '@' + user.split('@')[0])
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || 'El link del grupo ha cambiado a:\n@revoke').replace('@revoke', groupUpdate.revoke)
        if (!text) continue
